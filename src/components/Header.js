...
const Header = () => {
  ...
  return (
    <div className='absolute w-screen px-8 py-2 bg-gradient-to-b from-black z-10 flex justify-between'>
      ...
      {user && <h2 className='bg-gradient-to-r from-purple-600 via-red-500 to-indigo-400 inline-block text-transparent bg-clip-text text-2xl font-bold mt-4 ml-4 italic'>Welcome to MovieStore {profileName}</h2>}
      </div>
      ...
      {showSearchBar && <button className='text-white p-2 bg-red-500 hover:bg-opacity-80 rounded-lg my-3' onClick={handleCloseButton}>Close</button>} </div>}
      ...
      {user && <div className='flex'>
        ...
        <button className='py-2 px-4 bg-blue-500 bg-gradient-to-r from-indigo-500 my-3 mx-4 hover:opacity-80 text-white rounded-lg font-bold' onClick={handleSignOut}>Sign out</button>
      </div>}
    </div>
  )
}
